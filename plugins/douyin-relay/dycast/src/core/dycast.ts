...
export interface CastUser {
  ...
  // 性别 0 | 1 | 2 => 未知 | 男 | 女
  gender?: number;
  /** 粉丝团等级（user.fans_club.data），供中继映射为 blivechat medal_level */
  medalLevel?: number;
...
export class DyCast {
  ...
  /**
   * 从 User.fansClub 解析粉丝团展示信息。
   * 部分直播间仅下发 preferData（map），data 为空；此前只读 data 会导致勋章等级始终为 0。
   */
  private _pickFansClubDisplay(user?: User): { medalLevel: number; medalName: string } {
    const fc = user?.fansClub;
    if (!fc) return { medalLevel: 0, medalName: '' };

    const candidates: Array<{ level?: number; clubName?: string }> = [];
    if (fc.data) candidates.push(fc.data);
    const pd = fc.preferData;
    if (pd) {
    ...
    if (!candidates.length) return { medalLevel: 0, medalName: '' };

    let best = candidates[0];
    let bestLv = best.level ?? 0;
    for (let i = 1; i < candidates.length; i++) {
      ...
      if (lv > bestLv) {
        ...
        const bName = (best.clubName ?? '').trim();
        const cName = (c.clubName ?? '').trim();
        if (!bName && cName) best = c;
      }
    }

    const rawLv =
      best.level !== undefined && best.level !== null ? Number(best.level) : 0;
    const medalLevel = Number.isFinite(rawLv) ? rawLv : 0;
    const medalName =
      best.clubName !== undefined && best.clubName !== null
        ? String(best.clubName).trim()
        : '';
