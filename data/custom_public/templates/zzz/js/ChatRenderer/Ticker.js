...
function(constants, ImgShadow) {
  ...
  exports.default = {
    template: `
  <zzz-live-chat-ticker-renderer :hidden="showMessages.length === 0">
    <transition-group tag="div" :css="false" @enter="onTickerItemEnter" @leave="onTickerItemLeave"
      id="items" class="style-scope zzz-live-chat-ticker-renderer"
    >
      <zzz-live-chat-ticker-paid-message-item-renderer v-for="message in showMessages" :key="message.raw.id"
        tabindex="0" class="style-scope zzz-live-chat-ticker-renderer" style="overflow: hidden;"
      >
        <div id="container" dir="ltr" class="style-scope zzz-live-chat-ticker-paid-message-item-renderer" :style="{
          background: message.bgColor,
        }">
          <div id="content" class="style-scope zzz-live-chat-ticker-paid-message-item-renderer" :style="{
            color: message.color
          }">
            <img-shadow id="author-photo" height="24" width="24" class="style-scope zzz-live-chat-ticker-paid-message-item-renderer"
              :imgUrl="message.raw.avatarUrl"
            ></img-shadow>
            <span id="text" dir="ltr" class="style-scope zzz-live-chat-ticker-paid-message-item-renderer">{{ message.text }}</span>
          </div>
        </div>
      </zzz-live-chat-ticker-paid-message-item-renderer>
    </transition-group>
  </zzz-live-chat-ticker-renderer>
    `,
    name: 'Ticker',
    components: {
      ImgShadow
