...
function(constants, ImgShadow, AuthorChip) {
  ...
  exports.default = {
    template: `
  <zzz-live-chat-text-message-renderer :author-type="authorTypeText" :blc-guard-level="privilegeType">
    <img-shadow id="author-photo" height="24" width="24" class="style-scope zzz-live-chat-text-message-renderer"
      :imgUrl="avatarUrl"
    ></img-shadow>
    <div id="content" class="style-scope zzz-live-chat-text-message-renderer">
      <span id="timestamp" class="style-scope zzz-live-chat-text-message-renderer">{{ timeText }}</span>
      <author-chip class="style-scope zzz-live-chat-text-message-renderer"
        :isInMemberMessage="false" :authorName="authorName" :authorType="authorType" :privilegeType="privilegeType"
      ></author-chip>
      <span id="message" class="style-scope zzz-live-chat-text-message-renderer">
        <template v-for="(content, index) in contentParts">
          ...
          <img :key="index" v-else-if="content.type === CONTENT_PART_TYPE_IMAGE"
            class="emoji zzz-formatted-string style-scope zzz-live-chat-text-message-renderer"
  ...
  </zzz-live-chat-text-message-renderer>
    `,
    name: 'TextMessage',
    components: {
      ImgShadow,
